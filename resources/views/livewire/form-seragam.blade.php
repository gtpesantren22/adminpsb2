<div>
        ...
        <div class="bg-white p-5 rounded-lg shadow-xl flex flex-col items-center">
            <svg class="animate-spin h-10 w-10 text-primary-green mb-3" viewBox="0 0 50 50">
                <circle cx="25" cy="25" r="20" fill="none" stroke="currentColor" stroke-width="4"
                    stroke-linecap="round" stroke-dasharray="31.4 31.4" opacity="0.25" />
                <circle cx="25" cy="25" r="20" fill="none" stroke="#22c55e" stroke-width="4"
    ...
    <div class="glass-effect rounded-t-2xl p-6 text-center border-b border-gray-200">
        <div class="inline-block mb-3">
            <img src="{{ asset('logo/logo pp.png') }}" alt="Logo PP"
                class="h-16 mx-auto object-contain drop-shadow-md">
        </div>
    ...
    <div class="bg-white p-6 shadow-sm border-b border-gray-100">
        ...
        <div class="flex items-start space-x-3 mb-2">
            ...
            <div>
                ...
                <div
                    class="flex items-center text-xs font-semibold px-2 py-1 bg-blue-50 text-blue-600 rounded mt-1 inline-block">
        ...
        <div class="flex items-start space-x-3 mb-2 mt-4">
            ...
            <div>
                <p class="text-sm text-gray-600 leading-snug">
                    <span class="font-medium text-gray-700">Alamat:</span> {{ $santri->desa }} - {{ $santri->kec }} -
                    {{ $santri->kab }}
                </p>
            </div>
        </div>
    ...
    @if ($isSubmitted)
        <div class="bg-white p-6 rounded-b-2xl shadow-sm">
            <div class="bg-green-50 rounded-xl p-5 border border-green-100 text-center mb-6">
                ...
                <p class="text-sm text-green-600">Terima kasih, ukuran seragam ananda telah berhasil disimpan dalam
                    sistem kami.</p>
            </div>
        ...
        <div class="bg-white p-6 rounded-b-2xl shadow-sm">
            ...
            <div class="bg-blue-50/50 border border-blue-100 rounded-lg p-4 mb-6">
                ...
                <ul class="text-xs text-blue-700 space-y-2 list-disc pl-4">
                    <li>Untuk rincian panjang dan bahu bisa dicek keterangannya yang muncul di bawah saat memilih
                        ukuran.</li>
            ...
            <form wire:submit.prevent="save">
                ...
                <div class="mb-5">
                    ...
                    <div class="relative">
                        <select wire:model.live="atasan_id"
                            class="block w-full rounded-lg border-gray-300 shadow-sm focus:border-green-500 focus:ring-green-500 bg-gray-50 py-3 pl-4 pr-10 text-base">
                    ...
                    @if ($detailAtasan)
                        <div class="mt-3 bg-blue-50 border border-blue-200 rounded-lg p-4 shadow-inner">
                            <h5 class="text-xs font-bold text-blue-800 uppercase mb-2 border-b border-blue-200 pb-1">
                                Detail Ukuran Baju</h5>
                            <div class="flex justify-between items-center mb-1">
                                <span class="text-sm text-blue-700">Lebar Dada:</span>
                                <span class="text-blue-900 font-bold text-base">{{ $detailAtasan->ld }} <span
                                        class="text-xs font-normal">cm</span></span>
                            </div>
                            <div class="flex justify-between items-center">
                                <span class="text-sm text-blue-700">Panjang:</span>
                                <span class="text-blue-900 font-bold text-base">{{ $detailAtasan->pj }} <span
                                        class="text-xs font-normal">cm</span></span>
                            </div>
                        </div>
                    @endif
                ...
                <div class="mb-5 border-t border-gray-100 pt-5">
                    ...
                    <div class="relative">
                        <select wire:model.live="bawahan_id"
                            class="block w-full rounded-lg border-gray-300 shadow-sm focus:border-green-500 focus:ring-green-500 bg-gray-50 py-3 pl-4 pr-10 text-base">
                    ...
                    @if ($detailBawahan)
                        <div class="mt-3 bg-indigo-50 border border-indigo-200 rounded-lg p-4 shadow-inner">
                            <h5
                                class="text-xs font-bold text-indigo-800 uppercase mb-2 border-b border-indigo-200 pb-1">
                                Detail Ukuran Bawahan</h5>
                            <div class="flex justify-between items-center mb-1">
                                <span class="text-sm text-indigo-700">Lingkar Pinggang:</span>
                                <span class="text-indigo-900 font-bold text-base">{{ $detailBawahan->lp }} <span
                                        class="text-xs font-normal">cm</span></span>
                            </div>
                            <div class="flex justify-between items-center">
                                <span class="text-sm text-indigo-700">Panjang:</span>
                                <span class="text-indigo-900 font-bold text-base">{{ $detailBawahan->pj }} <span
                                        class="text-xs font-normal">cm</span></span>
                            </div>
                        </div>
                    @endif
                ...
                @if (strtoupper(substr($santri->jkl, 0, 1)) === 'L')
                    <div class="mb-6 border-t border-gray-100 pt-5">
                        ...
                        @error('songkok')
                            <span class="text-red-500 text-xs mt-1 block">{{ $message }}</span>
                        @enderror
                        <span class="text-xs text-gray-400 mt-1 block">Jika santri menggunakan ukuran 5 pada songkok
                            hitam nasional, maka pilihlah songkok No. 6</span>
                    </div>
                @endif

                <div class="pt-4 border-t border-gray-100">
                    <button type="submit"
                        class="w-full bg-green-600 hover:bg-green-700 text-white font-bold py-3.5 px-4 rounded-xl shadow-md transition duration-200 ease-in-out transform hover:-translate-y-0.5 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500">
                        <i class="fas fa-save mr-2"></i> Simpan Pilihan
                    </button>
                    <p class="text-center text-xs text-gray-400 mt-4"><i class="fas fa-lock mr-1"></i> Data Anda
