                ...
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">JKL</th>
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Gel</th>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @forelse ($datas as $row)
                            <tr>
                                ...
                                <td class="px-6 py-3">{{ $row->jkl }}</td>
                                <td class="px-6 py-3">{{ $row->gel }}</td>
                                <td class="px-6 py-3">{{ $row->hp }}</td>
                                <td class="px-6 py-3">{{ $row->lembaga }}</td>

                                <td class="px-6 py-3 whitespace-nowrap">
                                    <button wire:click.prevent="sync('{{ $row->id_santri }}')"
                                        wire:loading.attr="disabled"
                                        class="text-green-600 hover:text-green-800 mr-3 inline-flex items-center justify-center min-w-[20px]"
                                        title="Sinkron Ulang Data">
                                        <!-- SPINNER -->
                                        <svg wire:loading wire:target="sync('{{ $row->id_santri }}')"
                                            class="w-4 h-4 animate-spin text-green-600" viewBox="0 0 24 24"
                                            fill="none">
                                        ...
                                        <i wire:loading.remove wire:target="sync('{{ $row->id_santri }}')"
